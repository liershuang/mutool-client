        ...
        try {
            if (StrUtil.isNotEmpty(saveFilePath)) {
                ...
                if (!CONFIG_FILE.exists()) {
                    FileUtil.touch(CONFIG_FILE);
                }
                Yaml yaml = new Yaml();
                List<Map<String, String>> list = yaml.load(FileUtil.readString(CONFIG_FILE, "UTF-8"));
                if (list != null) {
    ...
    public void saveConfigure() {
        if (StrUtil.isNotEmpty(saveFilePath)) {
            try {
                File CONFIG_FILE = new File(saveFilePath);
                Yaml yaml = new Yaml();
                FileUtil.writeUtf8String(yaml.dump(this.getTableData()), CONFIG_FILE);
